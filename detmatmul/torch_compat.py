...
def _det_addmm_torch(
    ...
    *,
    ...
    """
    Deterministic replacement for torch.addmm(bias, A, B).
    Most PyTorch Linear layers dispatch here rather than to torch.mm.
    beta * bias + alpha * (A @ B)
    """
    A_np = _to_numpy_f32(A)
    B_np = _to_numpy_f32(B)
    C_np, _, _ = _matmul_raw(A_np, B_np, skip_overflow_check=True)
    result = _to_torch(C_np, A)
    if alpha != 1:
        result = result * alpha
    if bias is not None:
        b = bias
...
def _det_f_linear(
    ...
    """
    Deterministic replacement for torch.nn.functional.linear.

    F.linear computes: input @ weight.T + bias
    Handles 2D (batch, in) and 3D (batch, seq, in) inputs.
    """
    if not _TORCH_AVAILABLE:
        raise RuntimeError("PyTorch not available.")

    orig_shape = input.shape
    # Flatten to 2D for the matmul
    x2d   = input.reshape(-1, input.shape[-1])
    x_np  = _to_numpy_f32(x2d)
    w_np  = _to_numpy_f32(weight)
    C_np, _, _ = _matmul_raw(x_np, w_np.T, skip_overflow_check=True)
    result = _to_torch(C_np, input)
...
def _det_matmul_torch(
    ...
    # We must use _ORIGINAL_MATMUL to avoid infinite recursion since
    # torch.matmul is currently pointing at this function.
    if _ORIGINAL_MATMUL is not None:
        return _ORIGINAL_MATMUL(A, B)
    # Should never reach here — guard against calling before patch_torch()
    raise RuntimeError(
        "_det_matmul_torch called before patch_torch(). "
        "Use deterministic_mode() context manager instead."
    )
...
def patch_torch(verbose: bool = True) -> None:
    ...
    _ORIGINAL_F_LINEAR = _F.linear

    torch.mm     = _det_mm_torch
    torch.matmul = _det_matmul_torch
    torch.bmm    = _det_bmm_torch
    torch.addmm  = _det_addmm_torch
    _F.linear    = _det_f_linear
